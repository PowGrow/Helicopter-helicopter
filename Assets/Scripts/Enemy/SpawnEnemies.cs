...
public class SpawnEnemies : MonoBehaviour
{
    ...
    {
        var script = obj.GetComponent<DroidEnemy>();
        script.start = true;
        script._speed *= (1 + (float)(startCountOfEnemy - countEnemy) / 100);
        script.Health *= (1 + (float)(startCountOfEnemy - countEnemy)*1.4f / 100);
        script.damage *= (1 + (float)(startCountOfEnemy - countEnemy) * 1.3f / 100);
