...
public class Guns : MonoBehaviour, IShooter
{
    ...
    public void SetupGunMountPoints(int gunsSummCount, List<Vector2> wingsMountPoints)    //”станавливаем все пушки на нужные места в зависимости от их количества
    {
        //—начала отключаем все пушки
        foreach(GameObject gun in _gunsGameObjectList)
        {
            gun.SetActive(false);
        }
        //«атем активируем тольно нужные и устанавливаем им нужное положение
    ...
    private void LoadGunInfo()    //«агружает информацию о пушках при старте
    {
        _gunInfoList.Clear();
